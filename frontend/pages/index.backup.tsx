...
const HomeScreen: React.FC<Props> = ({ articles: initialArticles = [], fallback = false, error, tags = [] }) => {
  ...
  // Calculate category counts immediately from initial articles
  function calculateCategoryCounts(articles: Article[], tagList: any[]): Record<string, number> {
  ...
  const [categoryArticleCounts, setCategoryArticleCounts] = useState<Record<string, number>>(() => 
    calculateCategoryCounts(initialArticles.map(cleanArticle), tags)
  );
  ...
  // Recalculate counts when articles or tags change
  useEffect(() => {
    setCategoryArticleCounts(calculateCategoryCounts(allArticles, tagList));
  }, [allArticles, tagList]);
  // Pagination
  ...
  const filteredArticles = useMemo(() => {
    ...
    return allArticles.filter(a => {
      ...
      const text = [a.title, a.summary, a.source].filter(Boolean).map(v => String(v).toLowerCase()).join(' ');
      ...
      const matchesCategory = selectedCategories.some(slug => slug === articleSlug || tags.includes(slug));
      return matchesLang && matchesSearch && matchesCategory;
    });
  }, [allArticles, searchQuery, selectedLang, selectedCategories]);
  ...
  useEffect(() => {
    setCurrentPage(p => Math.min(totalPages, Math.max(1, p)));
  }, [totalPages]);

  const getPageRange = (total: number, current: number, windowSize: number) => {
